...
from ..models.end_to_end.patch_borehole_cls_transformer import (
...
@dataclass
class PatchBoreholeCLSE2ETrainingConfig:
    """Training hyperparameters for PatchBoreholeCLSEndToEndMapBeliefTransformer.

    Identical to PatchBoreholeE2ETrainingConfig — only the model class and
    checkpoint names differ.
    """
    ...
    # Spatial grid — set automatically from cache in train_patch_borehole_cls_transformer()
    n_x: int = 32
    n_y: int = 32
...
def train_patch_borehole_cls_transformer(
    ...
    cfg: PatchBoreholeCLSE2ETrainingConfig,
    device: str,
    checkpoint_dir: Path,
    ...
    save_checkpoint_model(
        checkpoint_dir / "patch_borehole_cls_last.pt",
        model,
        cfg,
        epoch,
        history,
...
def load_patch_borehole_cls_checkpoint(
    path: Path,
    device: str = "cpu",
) -> tuple[PatchBoreholeCLSEndToEndMapBeliefTransformer, PatchBoreholeCLSE2ETrainingConfig, TargetNormalizer, list[dict]]:
    ...
    return load_model_encoder_checkpoint(
        path, _model_fn, PatchBoreholeCLSE2ETrainingConfig, device
    )
