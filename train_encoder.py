...
def stream_batches_from_dir(
    ...
    if not map_files:
        raise RuntimeError(
            f"no boreholes_*.npy files in {dataset_dir} " f"(did you run pull_maps.py?)"
        )
    ...
    if recompute_stats_n_maps is not None:
        print(
            f"  refining standardisation stats from "
...
class _TrainLog:
    ...
    def plot(self, title: str = "", best_step: int | None = None) -> None:
        ...
        loss_fields = [f for f in self._fields if f in ("loss", "loss_ema")]
        extra_fields = [
            f for f in self._fields if f not in ("step",) + tuple(loss_fields)
        ]
        ...
        for fld in loss_fields:
            ...
            ax0.plot(steps_all, ys, label=fld, lw=1.0 if fld != "loss_ema" else 1.6)
        ax0.set_ylabel("loss")
        ax0.grid(alpha=0.3)
        if loss_fields:
        ...
        if best_step is not None:
            ax0.axvline(
                best_step, color="grey", lw=0.8, ls="--", label=f"best @ {best_step}"
            )
        ...
        if extra_fields:
            ...
            for fld in extra_fields:
                pts = [
                    (r["step"], r[fld])
                    for r in ordered
                    if fld in r and r[fld] == r[fld]
                ]  # filter NaN
                if not pts:
                    continue
                xs, ys = zip(*pts)
                ax1.plot(xs, ys, marker="o", ms=2.5, lw=0.9, label=fld)
            ax1.set_ylabel("aux metrics")
            ax1.grid(alpha=0.3)
            ax1.legend(fontsize=7, ncol=min(len(extra_fields), 4), loc="upper right")
...
def _dump_profile(
    pr: cProfile.Profile, dump_path: Path | None, top_n: int = 20
) -> None:
...
def train(
    ...
    print(
        f"AMP: {'bf16' if (use_amp and amp_dtype is torch.bfloat16) else ('fp16' if use_amp else 'off')}  "
    ...
    if min_maps_warmup is not None:
        ...
        print(
            f"early-stop warmup pinned to {min_maps_warmup} maps × "
    ...
    print(
        f"early stopping: {'on' if stopper.enabled else 'off'} "
    ...
    for step in range(1, steps + 1):
        ...
        x = next(batches)
        with torch.autocast(
            device_type="cuda" if use_amp else "cpu", dtype=amp_dtype, enabled=use_amp
        ):
            recon, z = model(x)
        ...
        loss_running = (
            0.95 * loss_running + 0.05 * loss.item() if step > 1 else loss.item()
        )

        if step % log_every == 0 or step == 1:
            print(
                f"  step {step:>5d}  loss={loss.item():.4f}  (ema {loss_running:.4f})"
            )
            ...
            with torch.no_grad():
                ...
                for i, v in enumerate(variables):
                    l = nn.functional.smooth_l1_loss(
                        recon[:, i, :], x[:, i, :], beta=1.0
                    ).item()
                    per_var.append((v, l))
            print(f"    per-variable: " + "  ".join(f"{v}={l:.2f}" for v, l in per_var))
        ...
        if should_stop:
            print(
                f"  early stop at step {step}: no improvement for "
    ...
    if best_state is not None and (
        last_step != stopper.best_step or stopper.stale_checks
    ):
        _underlying(model).load_state_dict(best_state)
        print(
            f"  restored best model state (ema {stopper.best:.4f} @ step {stopper.best_step})"
        )
...
def main():
    ...
    if dataset_dir is not None and str(dataset_dir) == "":
        ...
        print(
            f"  note: --dataset-dir {dataset_dir} does not exist, "
